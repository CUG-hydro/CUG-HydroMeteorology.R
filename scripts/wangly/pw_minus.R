...
time1 <- as.PCICt(x = unit, cal = "gregorian") + time*3600
time1[1]
...
write_tc <- function(lon, lat, pw, p,outname){
        ...
        var.pw <- ncvar_def(name = 'pw', units = 'kg/m2', missval = NA,
                             dim = list(lon, lat), longname = 'pw - warm minus cold')
        var.p <- ncvar_def(name = 'p', units = '', missval = NA,
...
write_tc(lon = lon, lat = lat, pw = pw, p = p,
         outname = ".\\outputted data\\sst_etc\\pw_minus.nc")
